local crt_lab_veh_hall = {}

shuffle = function(set)
  for i = #set, 1, -1 do
    local j = gapi.rng(1, i)
    set[i], set[j] = set[j], set[i]
  end
  return set
end

supplies_gen = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_storage_pantry_food",
    "crt_lab_veh_storage_frozen_food",
    "crt_lab_veh_storage_recreation",
    "crt_lab_veh_storage_misc_repair",
    "crt_lab_veh_storage_uniforms",
    "crt_lab_veh_storage_vault",
  })
  draw_hall_set(data, map, rooms, point)
end

supplies_eng = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_storage_chem",
    "crt_lab_veh_storage_tools",
    "crt_lab_veh_storage_hardware",
    "crt_lab_veh_storage_electronics",
    "crt_lab_veh_storage_testing_mats",
    "crt_lab_veh_storage_hazmat",
  })
  draw_hall_set(data, map, rooms, point)
end

supplies_mil = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_storage_explosives",
    "crt_lab_veh_storage_armory",
    "crt_lab_veh_storage_hack",
    "crt_lab_veh_storage_training",
    "crt_lab_veh_storage_field_med",
    "crt_lab_veh_storage_mildrugs",
  })
  draw_hall_set(data, map, rooms, point)
end

engine = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_part_production",
    "crt_lab_veh_hall_engine",
    "crt_lab_veh_hall_engine_test",
    "crt_lab_veh_smallveh",
  })
  draw_hall_set(data, map, rooms, point)
end

gen_amenities = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_milfood",
    "crt_lab_veh_hall_scifood",
    "crt_lab_veh_hall_exercise",
    "crt_lab_veh_hall_dodge_train",
    "crt_lab_veh_hall_library",
    "crt_lab_veh_hall_pool",
    "crt_lab_veh_hall_living",
    "crt_lab_veh_hall_hydroponics",
    "crt_lab_veh_hall_bath_good",
    "crt_lab_veh_hall_bath_mil",
  })
  draw_hall_set(data, map, rooms, point)
end

mil_amenities = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_milfood",
    "crt_lab_veh_hall_exercise",
    "crt_lab_veh_hall_dodge_train",
    "crt_lab_veh_hall_bath_mil",
  })
  draw_hall_set(data, map, rooms, point)
end

sci_amenities = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_scifood",
    "crt_lab_veh_hall_living",
    "crt_lab_veh_hall_library",
    "crt_lab_veh_hall_hydroponics",
  })
  draw_hall_set(data, map, rooms, point)
end

offices = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_office_paperwork",
    "crt_lab_veh_hall_office_group_med",
    "crt_lab_veh_hall_office_group_eng",
    "crt_lab_veh_hall_office_cubicles",
  })
  draw_hall_set(data, map, rooms, point)
end

doctor = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_doctor_storage",
    "crt_lab_veh_hall_exercise",
    "crt_lab_veh_hall_doctor_autodoc",
    "crt_lab_veh_hall_doctor_house",
  })
  draw_hall_set(data, map, rooms, point)
end

all = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_milfood",
    "crt_lab_veh_hall_scifood",
    "crt_lab_veh_hall_exercise",
    "crt_lab_veh_hall_library",
    "crt_lab_veh_hall_living",
    "crt_lab_veh_hall_hydroponics",
    "crt_lab_veh_hall_dodge_train",
    "crt_lab_veh_hall_bath_good",
    "crt_lab_veh_hall_bath_mil",
    "crt_lab_veh_hall_engine",
    "crt_lab_veh_hall_engine_test",
    "crt_lab_veh_smallveh",
  })
  draw_hall_set(data, map, rooms, point)
end

hydrogen = function(data, map, point)
  rooms = shuffle({
    "crt_lab_veh_hall_hydrogen_storage",
    "crt_lab_veh_hall_hydrogen_storage",
    "crt_lab_veh_hall_hydrogen_storage",
    "crt_lab_veh_hall_hydrogen_storage",
  })
  draw_hall_set(data, map, rooms, point)
end

local halls = { engine, all, gen_amenities, mil_amenities, sci_amenities, doctor, offices, hydrogen }

local halls_weight = { 100, 200, 50, 75, 75, 75, 125, 200 }

local total_weight = 0

for _, weight in ipairs(halls_weight) do
  total_weight = total_weight + weight
end

draw_hall_set = function(data, map, set, pos)
  data:nest(set[1], PointRelMs.new(pos.x, pos.y))
  data:nest(set[2], PointRelMs.new(pos.x + 15, pos.y))
  map:rotate(2)

  data:nest(set[3], PointRelMs.new(pos.x, pos.y))
  data:nest(set[4], PointRelMs.new(pos.x + 15, pos.y))
  map:rotate(2)
end

crt_lab_veh_hall.draw = function(data, map)
  map:rotate(4 - data:get_rotation())
  data:generate("crt_lab_veh_hall_frame")
  -- Generate rotates the map but we need unrotated maps for nested generation
  -- Rotation has 4 states, you rotate 3 need 1 more to get back to normal and so on
  local weight = gapi.rng(0, total_weight)
  local count = 0
  local hall
  for i, add in ipairs(halls_weight) do
    count = count + add
    if count >= weight then
      hall = halls[i]
      break
    end
  end
  hall(data, map, PointRelMs.new(1, 1))

  -- Now that all the nested is dealt with rerotate the map
  map:rotate(data:get_rotation())
end

return crt_lab_veh_hall
